...
CREATE OR REPLACE FUNCTION public.is_business_admin(_user_id UUID)
RETURNS BOOLEAN
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.user_roles WHERE user_id = _user_id AND role IN ('owner', 'admin')
  )
$$;
...
-- BUSINESSES
CREATE POLICY "Users can view own business" ON public.businesses FOR SELECT TO authenticated
  USING (id = public.current_user_business_id());
CREATE POLICY "Owners can update business" ON public.businesses FOR UPDATE TO authenticated
  USING (id = public.current_user_business_id() AND public.has_role(auth.uid(), 'owner'));

-- PROFILES
CREATE POLICY "Users can view profiles in their business" ON public.profiles FOR SELECT TO authenticated
  USING (business_id = public.current_user_business_id());
CREATE POLICY "Users can insert own profile" ON public.profiles FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update own profile" ON public.profiles FOR UPDATE TO authenticated
  USING (user_id = auth.uid() AND business_id = public.current_user_business_id());
...
-- CUSTOMERS
CREATE POLICY "Business members can view customers" ON public.customers FOR SELECT TO authenticated
  USING (business_id = public.current_user_business_id());
CREATE POLICY "Admins+ can create customers" ON public.customers FOR INSERT TO authenticated
  WITH CHECK (business_id = public.current_user_business_id() AND public.is_business_admin(auth.uid()));
CREATE POLICY "Admins+ can update customers" ON public.customers FOR UPDATE TO authenticated
  USING (business_id = public.current_user_business_id() AND public.is_business_admin(auth.uid()));
CREATE POLICY "Owners can delete customers" ON public.customers FOR DELETE TO authenticated
  USING (business_id = public.current_user_business_id() AND public.has_role(auth.uid(), 'owner'));
...
-- ORDERS
CREATE POLICY "Business members can view orders" ON public.orders FOR SELECT TO authenticated
  USING (business_id = public.current_user_business_id());
...
CREATE POLICY "View order items via order" ON public.order_items FOR SELECT TO authenticated
  USING (order_id IN (SELECT id FROM public.orders WHERE business_id = public.current_user_business_id()));
CREATE POLICY "Create order items" ON public.order_items FOR INSERT TO authenticated
  WITH CHECK (order_id IN (SELECT id FROM public.orders WHERE business_id = public.current_user_business_id()));
...
CREATE POLICY "Delete order items" ON public.order_items FOR DELETE TO authenticated
  USING (order_id IN (SELECT id FROM public.orders WHERE business_id = public.current_user_business_id()) AND public.has_role(auth.uid(), 'owner'));
...
CREATE POLICY "Update conversations" ON public.conversations FOR UPDATE TO authenticated
  USING (business_id = public.current_user_business_id());
CREATE POLICY "Delete conversations" ON public.conversations FOR DELETE TO authenticated
  USING (business_id = public.current_user_business_id() AND public.has_role(auth.uid(), 'owner'));

-- MESSAGES
CREATE POLICY "View messages" ON public.messages FOR SELECT TO authenticated
  USING (conversation_id IN (SELECT id FROM public.conversations WHERE business_id = public.current_user_business_id()));
CREATE POLICY "Create messages" ON public.messages FOR INSERT TO authenticated
  WITH CHECK (conversation_id IN (SELECT id FROM public.conversations WHERE business_id = public.current_user_business_id()));

-- ANALYTICS_LOGS
CREATE POLICY "View analytics" ON public.analytics_logs FOR SELECT TO authenticated
  USING (business_id = public.current_user_business_id());
CREATE POLICY "Create analytics" ON public.analytics_logs FOR INSERT TO authenticated
  WITH CHECK (business_id = public.current_user_business_id());

-- DEMAND_PREDICTIONS
CREATE POLICY "View predictions" ON public.demand_predictions FOR SELECT TO authenticated
  USING (business_id = public.current_user_business_id());
...
BEGIN
  -- Create a new business for every signup
  ...
  VALUES (NEW.id, new_business_id, COALESCE(NEW.raw_user_meta_data->>'full_name', ''));
